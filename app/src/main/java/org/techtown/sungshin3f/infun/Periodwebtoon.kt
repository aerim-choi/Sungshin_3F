...
import org.techtown.sungshin3f.databinding.ActivityPeriodwebtoonBinding
import kotlin.concurrent.thread

class Periodwebtoon : AppCompatActivity() {
    private var mBinding: ActivityPeriodwebtoonBinding?=null
    private val binding get()=mBinding!!
    val GenWebtoonImgs=ArrayList<String>()
    val GenWebtoonTitles=ArrayList<String>()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mBinding = ActivityPeriodwebtoonBinding.inflate(layoutInflater)
        setContentView(binding.root)

        GenreWebtoonPerInfo()
    }
    private fun GenreWebtoonPerInfo(){
        thread {
            val doc = Jsoup.connect("https://comic.naver.com/webtoon/genre?genre=historical")
                .get()
