...
import org.techtown.sungshin3f.databinding.ActivityFantasywebtoonBinding
import org.techtown.sungshin3f.R
import kotlin.concurrent.thread

class Fantasywebtoon : AppCompatActivity() {
    private var mBinding: ActivityFantasywebtoonBinding?=null
    private val binding get()=mBinding!!
    val GenWebtoonImgs=ArrayList<String>()
    val GenWebtoonTitles=ArrayList<String>()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_fantasywebtoon)
        mBinding = ActivityFantasywebtoonBinding.inflate(layoutInflater)
        setContentView(binding.root)
        GenreWebtoonFanInfo()
    }
    private fun GenreWebtoonFanInfo(){
        thread {
            val doc = Jsoup.connect("https://comic.naver.com/webtoon/genre?genre=fantasy")
                .get()
