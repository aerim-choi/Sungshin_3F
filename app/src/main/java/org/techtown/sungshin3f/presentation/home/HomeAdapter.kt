...
import org.techtown.sungshin3f.databinding.ItemFeaturedMovieBinding
import org.techtown.sungshin3f.databinding.ItemMovieBinding
...
class HomeAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    fun addData(featuredMovie: FeaturedMovie?, movies: List<Movie>) {
        ...
        featuredMovie?.let {
            newData += DataItem("🔥수정이를 위한 오늘의 추천")
            newData += DataItem(it)
        }

        newData += DataItem("🍿이 연극들은 보셨나요?")
